# Zap videos — what to shoot, and what to generate
...
- Downloads folder cleared except the Zap zip.
...
|---|---|---|
| 0:00–0:05 | The downloaded zip in Downloads. Double-click it. Folder `zap` appears. | "1. Unzip it" |
| 0:05–0:12 | Drag the `zap` folder into Documents. Pause a beat on it. | "Keep this folder — don't delete it" |
...
| 0:27–0:36 | Click **Load unpacked**. File picker opens. Select the `zap` folder — **highlight that you pick the folder itself, not `src`**. Click Select. | "4. Load unpacked → pick the zap folder" |
| 0:36–0:42 | The Zap card appears. Pin it. Popup opens showing live deals. | "Done — your deals are in the toolbar" |
| 0:42–0:45 | Popup with deals, cursor hovering one. | "indiaoffers.in/zap" |
...
0:14  caption "Zap checks out for you."
0:18  end card: Zap logo + indiaoffers.in/zap   (2s, static)
```
...
- **Screen recording** → embedded on `/zap` under step 4, and linked from
...
- **The GIF** → inline on `/zap` at step 3, no click needed.
